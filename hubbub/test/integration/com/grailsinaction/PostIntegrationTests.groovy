...
class PostIntegrationTests extends GroovyTestCase {
	...
	void testFirstPost(){
		def user = new User(userId: '1234567B', password: 'secret')
		user.save()
	...
	void testAccessingPosts(){
		def user = new User(userId: '1234567B', password: 'secret').save()
	...
	void testPostWithTags(){
		...
		bothPost.addToTags(groovy)
		bothPost.addToTags(grails)
		assetEquals 2, bothPost.tags.size()
	}
}
